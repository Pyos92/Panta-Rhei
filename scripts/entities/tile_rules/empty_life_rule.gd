extends AbstractTileLifeRule
class_name EmptyLifeRule

const OVERGROWTH_CONTROL := true

func evaluate_next_gen(tile_to_evaluate: Tile) -> void:
	if tile_to_evaluate.next_step_action != Tile.CellNextStep.IDLE: 
		return
	# Controllo se può far nascere alberi
	var flowers_2_tree : Array[Tile] = get_tiles_in_area(tile_to_evaluate,TreeLifeRule.TREE_SEARCH_GROW_AREA, Tile.CellType.FLOWER)
	if flowers_2_tree.size() >= 4 and _has_space_to_grow_tree(tile_to_evaluate):
		tile_to_evaluate.set_next_step_action(Tile.CellNextStep.GROW_TREE)
		return
	
	# Controllo se può far nascere fiori
	var flowers : Array[Tile] = get_tiles_in_area(tile_to_evaluate,PROXIMITY_SEARCH_AREA, Tile.CellType.FLOWER)
	if flowers.size() == 3:
		tile_to_evaluate.set_next_step_action(Tile.CellNextStep.GROW_FLOWER)
		return
	
	#Conto gli elementi vitali e ignoro lo spawn di animali se nell'area sta già per nascerne 1
	var tiles : Array[Tile] = get_tiles_in_area(tile_to_evaluate,AnimalLifeRule.ANIMAL_SEARCH_AREA, null)
	var count = 0
	for t in tiles: 
		if OVERGROWTH_CONTROL and t.next_step_action == Tile.CellNextStep.GROW_ANIMAL: 
			count = -1
			break
		count += t.type
	if count >= 30:
		tile_to_evaluate.set_next_step_action(Tile.CellNextStep.GROW_ANIMAL)
		return
